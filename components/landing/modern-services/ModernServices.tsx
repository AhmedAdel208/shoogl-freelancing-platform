...
export default function ModernServices({ t, isRtl }: ModernServicesProps) {
  return (
    <section className="py-16 sm:py-20 md:py-24 bg-bg select-none overflow-hidden">
      <div className="max-w-8xl mx-auto px-6 md:px-12">
        ...
        >
          <div
            className={`space-y-4 sm:space-y-5 max-w-2xl w-full ${isRtl ? "text-right" : "text-left"}`}
          >
              ...
              <SwiperSlide key={service.id}>
                ...
                >
                  ...
                  <div className="p-7">
                    {/* Author Info */}
                    <div
                      className={`flex items-center gap-3 mb-5 ${isRtl ? "flex-row" : "flex-row-reverse justify-end"}`}
                    >
                      <div className="w-11 h-11 rounded-[14px] bg-bg border border-border overflow-hidden relative shadow-inner">
                        {service.personImage ? (
                          ...
                          <User
                            size={20}
                            className="text-gray-medium/30 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"
                          />
                        )}
                      </div>
                    ...
                    <h3
                      className={`text-[19px] font-black font-cairo text-heading mb-3 line-clamp-1 h-7 ${isRtl ? "text-right" : "text-left"}`}
                    >
                      {isRtl ? service.title : service.titleEn || service.title}
                    </h3>

                    <p
                      className={`text-sm font-bold font-cairo text-gray-medium leading-relaxed line-clamp-2 mb-6 h-10 ${isRtl ? "text-right" : "text-left"}`}
                    >
                      {isRtl
                        ? service.description
                    ...
                    {/* Footer Metrics */}
                    <div
                      className={`pt-6 border-t border-border flex items-center justify-between ${isRtl ? "flex-row" : "flex-row-reverse"}`}
                    >
                      <div className="flex items-center gap-2 text-primary font-black font-cairo">
                        <Wallet size={16} />
                        <span className="text-xl">
                          {service.price}{" "}
                          <span className="text-[10px] font-bold text-gray-medium/60">
                            {t.common.riyal}
                          </span>
                        </span>
                      </div>
                      ...
                      <div
                        className={`flex items-center gap-1.5 text-gray-medium font-bold font-cairo text-xs ${isRtl ? "" : "flex-row-reverse"}`}
                      >
        ...
        <div className="mt-20 sm:mt-24 md:mt-32 p-6 sm:p-10 lg:p-16 xl:p-20 bg-card-bg rounded-[40px] md:rounded-[60px] relative overflow-hidden group border border-border shadow-md">
          ...
          <div
            className={`relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-10 sm:gap-12 lg:gap-16 items-center ${isRtl ? "" : "text-left"}`}
          >
            <div
            ...
            >
              ...
              <h3 className="text-3xl sm:text-4xl md:text-[40px] font-black text-heading font-cairo leading-tight sm:leading-[1.2]">
                {isRtl ? (
                  ...
                  <>
                    Why Choose{" "}
                    <span className="text-primary italic">SHOGOL</span>
                  </>
                )}
              </h3>
              ...
              {[
                {
                  ...
                  desc: isRtl
                    ? "نستخدم أحدث تقنيات التشفير لضمان سرية معلوماتك المالية والشخصية."
                    : "We use the latest encryption technologies to ensure the confidentiality of your financial and personal info.",
                  color:
                ...
                },
                {
                  ...
                  desc: isRtl
                    ? "نخبة من المستقلين الذين تم اختيارهم بعناية لضمان أعلى جودة في التنفيذ."
                    : "A selection of freelancers carefully chosen to ensure the highest quality of execution.",
                ...
                },
                {
                  ...
                  desc: isRtl
                    ? "نظام حماية الدفعات يضمن حقوق الطرفين حتى إتمام العمل بنجاح."
                    : "The payment protection system guarantees the rights of both parties until the work is successfully completed.",
                ...
                },
                {
                  ...
                  desc: isRtl
                    ? "فريق دعم متخصص متواجد دائماً لمساعدتك في أي وقت وبكل احترافية."
                    : "A specialized support team is always available to assist you at any time with full professionalism.",
                ...
                >
                  <div
                    className={`w-12 h-12 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center mb-4 sm:mb-6 ring-1 ${box.color}`}
                  >
                    {box.icon}
                  </div>
                  <h4
                    className={`text-heading font-black font-cairo text-lg sm:text-xl mb-2 sm:mb-3 ${isRtl ? "text-right" : "text-left"}`}
                  >
                    {box.title}
                  </h4>
                  <p
                    className={`text-gray-medium text-sm sm:text-[15px] font-bold font-cairo leading-relaxed opacity-80 ${isRtl ? "text-right" : "text-left"}`}
                  >
                    {box.desc}
                  </p>
                </div>
